...
for i in range(31,46):
    uri = 'https://github.com/search?o=asc&p='+str(i+1)+'&q=stars%3A%3E10000&s=stars&type=Repositories'
    driver.get(uri)
    time.sleep(0.8)
    repos = driver.find_elements_by_xpath("/html/body/div[5]/main/div[1]/div[3]/div[1]/ul[1]/li")
    origin_window = driver.current_window_handle
    for j in range(len(repos)):
        name = driver.find_element_by_xpath("/html/body/div[5]/main/div[1]/div[3]/div[1]/ul[1]/li["+str(j+1)+"]/div[2]/div[1]/div[1]/a").text
        repo = name.split("/")[-1]
        if os.path.exists('dockerfile\\'+repo):
            continue
